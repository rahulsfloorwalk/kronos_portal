...
def get_auditor_list_by_filter_for_pincode(filters: dict) -> list:
    query={
        'is_active': True,
    }
    
    if filters.get('channel_name')=="email":
        query['preferences__receive_new_opportunities_email']=True
    
    if filters.get('gender',''):
        query['profileinfo__gender__in'] = filters.get('gender','')

    if filters.get('pincode', ''):
        query['profileinfo__pincode'] = filters.get('pincode', '')
