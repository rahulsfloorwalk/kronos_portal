...
import {fetchRegionWisePerformanceByAuditCycleId} from "../service/dashboard.js";
...
class RegionWisePerformanceChart extends React.Component{
	static propTypes = {
	...
	downloadFile = () =>{
		...
		domtoimage.toBlob(document.getElementById(`region_performance_${this.props.type}`), {bgcolor: "white"})
			.then(function (blob) {
				fileDownload(blob,  audit_cycle_name + " Region Wise Performance.jpg");
			});
	};
	...
	render(){
		...
		let chart = (
			<ResponsiveContainer width="100%" aspect={3 / 1} id={`region_performance_${this.props.type}`}>
				<BarChart width={600} height={300} data={data} margin={{top: 25, right: 10, left: 10, bottom: 5}} onClick={(active)=>active&&this.toggleModal()}>
					...
					<XAxis dataKey="name" tick={this.tickFunction} interval={0}>
						<Label value="Region List" offset={0} position="insideBottomRight" />
					</XAxis>
...
export default class RegionWisePerformanceChartWrapper extends React.Component{
	static propTypes = {
	...
	reloadData = (questionnaireTypeId, auditCycleId) => {
		if(!demo){
			...
			fetchRegionWisePerformanceByAuditCycleId(questionnaireTypeId, auditCycleId).then((reportData) => {
				this.setState({
	...
	render(){
		if(this.state.loading || ! this.state.reportData ){
			...
			return (
				<div>
					<h3 className="text-center">Region Wise Performance</h3>
			...
			return (
				<div className="row">
					<div className="col-md-6">
						<RegionWisePerformanceChart title="Best Performing Region" type="best" reportData={this.state.reportData} auditCycle={this.props.auditCycle}/>
					</div>
					<div className="col-md-6">
						<RegionWisePerformanceChart title="Worst Performing Region" type="worst" reportData={this.state.reportData} auditCycle={this.props.auditCycle}/>
					</div>
				</div>
			);
		} else {
			return (
				<RegionWisePerformanceChart title="Region Wise Performance" type="all" reportData={this.state.reportData} auditCycle={this.props.auditCycle}/>
			);
		}
	}
}
