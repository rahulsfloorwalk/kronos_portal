...
import { QuestionnaireTypeTabsForDashboard } from "../QuestionnaireTypeTabsForDashboard";

describe(QuestionnaireTypeTabsForDashboard, () => {
	...
	it("renders a loading sign when there are no questionnaire types", () => {
		...
		const r = renderer.create(<QuestionnaireTypeTabsForDashboard
			onMount={onMount}
			onSelect={onSelect}
			questionnaireTypes={[]}
		/>);
	...
	it("calls onMount when it is mounted", () => {
		const onMount = jest.fn();
		const onSelect = jest.fn();
		renderer.create(<QuestionnaireTypeTabsForDashboard
			onMount={onMount}
			onSelect={onSelect}
			questionnaireTypes={sampleQuestionnaireTypes}
			selectedQuestionnaireType={sampleQuestionnaireTypes[1]}
	...
	it("calls onSelect when a tab is clicked", () => {
		const onMount = jest.fn();
		const onSelect = jest.fn();
		const r = shallow(<QuestionnaireTypeTabsForDashboard
			onMount={onMount}
			onSelect={onSelect}
			questionnaireTypes={sampleQuestionnaireTypes}
			selectedQuestionnaireType={sampleQuestionnaireTypes[1]}
