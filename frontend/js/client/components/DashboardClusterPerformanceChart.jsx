...
import {fetchClusterWisePerformanceByAuditCycleId} from "../service/dashboard.js";
...
class ClusterWisePerformanceChart extends React.Component{
	static propTypes = {
	...
	downloadFile = () =>{
		...
		domtoimage.toBlob(document.getElementById(`cluster_performance_${this.props.type}`), {bgcolor: "white"})
			.then(function (blob) {
				fileDownload(blob,  audit_cycle_name + " Cluster Wise Performance.jpg");
			});
	};
	...
	render(){
		...
		let chart = (
			<ResponsiveContainer width="100%" aspect={3 / 1} id={`cluster_performance_${this.props.type}`}>
				<BarChart width={600} height={300} data={data} margin={{top: 25, right: 10, left: 10, bottom: 5}} onClick={(active)=>active&&this.toggleModal()}>
					...
					<XAxis dataKey="name" tick={this.tickFunction} interval={0}>
						<Label value="Cluster List" offset={0} position="insideBottomRight" />
					</XAxis>
...
export default class ClusterWisePerformanceChartWrapper extends React.Component{
	static propTypes = {
	...
	reloadData = (questionnaireTypeId, auditCycleId) => {
		if(!demo){
			...
			fetchClusterWisePerformanceByAuditCycleId(questionnaireTypeId, auditCycleId).then((reportData) => {
				this.setState({
	...
	render(){
		if(this.state.loading || ! this.state.reportData ){
			...
			return (
				<div>
					<h3 className="text-center">Cluster Wise Performance</h3>
			...
			return (
				<div className="row">
					<div className="col-md-6">
						<ClusterWisePerformanceChart title="Best Performing Cluster" type="best" reportData={this.state.reportData} auditCycle={this.props.auditCycle}/>
					</div>
					<div className="col-md-6">
						<ClusterWisePerformanceChart title="Worst Performing Cluster" type="worst" reportData={this.state.reportData} auditCycle={this.props.auditCycle}/>
					</div>
				</div>
			);
		} else {
			return (
				<ClusterWisePerformanceChart title="Cluster Wise Performance" type="all" reportData={this.state.reportData} auditCycle={this.props.auditCycle}/>
			);
		}
	}
}
