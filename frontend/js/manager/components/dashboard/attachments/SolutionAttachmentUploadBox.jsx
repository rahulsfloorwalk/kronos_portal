...
import { findAttachmentsBySolution, deleteAttachment, uploadFileForSolutions, findSolutionById } from "../../../service/admin_dashboard.js";
import { Link } from "react-router";
import { Paperclip, Cross } from "../../../../components/Icons.jsx";
import SAttachmentInProgress from "./SAttachmentInProgress.jsx";
import SAttachmentThumbnail from "./SAttachmentThumbnail.jsx.jsx";
import SAttachmentPreview from "./SAttachmentPreview.jsx";
...
export default class SolutionAttachmentUploadBox extends React.Component {
	...
	render() {
		...
		for (let a of this.state.attachments) {
			attachmentRows.push(<SAttachmentThumbnail
				attachment={a}
				deletable={deletable}
		...
		for (let id in this.state.inProgress) {
			if (this.state.inProgress[id].uploading || this.state.inProgress[id].error) {
				...
				attachmentRows.push(<SAttachmentInProgress
					key={id}
					fileName={fileName}
		...
		let attachmentElement = <SAttachmentPreview attachment={this.state.selectedAttachment}
			section_id={0}
		...
		return (
			<div className={"panel " + panelClass}>
				...
				<div className="panel-heading" style={{ cursor: "pointer" }} onClick={this.toggleExpand}>
					...
					{uploadButton}
					<h4>
					Attachments for <b>{this.state.solution.name}</b><span className="text-danger" style={{ marginLeft: ".5rem" }}><b>*</b></span>
					</h4>
				</div>
				{toShow ?
					<div className="row">
