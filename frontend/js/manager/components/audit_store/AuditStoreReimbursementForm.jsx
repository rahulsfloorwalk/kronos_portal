...
import { setReimbursement } from "../../service/audit_store.js";
...
export class AuditStoreReimbursementForm extends Component{
	static propTypes = {
	...
	componentDidMount(){
		...
		if(this.props.auditStore){
			this.setState({
				"reimbursement": this.props.auditStore.reimbursement || this.props.auditStore.audit.reimbursement,
			});
		}
	}
	componentWillReceiveProps(nextProps) {
		if(nextProps.auditStore){
			this.setState({
				"reimbursement": nextProps.auditStore.reimbursement || nextProps.auditStore.audit.reimbursement,
			});
		}
	}
	fieldChanged = (e) => {
	...
	onSubmit = (e) => {
		e.preventDefault();
		let promise = this.props.onSubmit(this.props.params.auditStoreId, this.state.reimbursement);
		promise.then(()=>{
			this.props.router.push(`/audit_store/${this.props.params.auditStoreId}/report`);
			Alert.success("REIMBURSEMENT SAVED");
		}, (err)=>{
			this.setState({
	...
	render(){
		...
		return ( <Modal modalTitle={modalTitle} onClose={this.props.router.goBack}>
			<form onSubmit={this.onSubmit}>
				...
				<FormInput type="number" label="Reimbursement" value={this.state.reimbursement} name="reimbursement" onChange={this.fieldChanged} errors={this.state.errors.reimbursement}/>
...
const mapDispatchToProps = (dispatch) => {
	return {
		...
		onSubmit: (auditStoreId, reimbursement) => {
			const promise = setReimbursement(auditStoreId, reimbursement);
...
export default connect( mapStoreToProps, mapDispatchToProps)(AuditStoreReimbursementForm);
