	...
	render() {
		...
		return (
			<tr>
				...
				<td>{is_active}</td>
				<td>
					<Link to={`/projects/${this.props.clientUser.client.id}/client_user/${this.props.clientUser.id}/edit`} className="btn btn-default"><Pencil/></Link>
					&nbsp;
					{!this.props.clientUser.is_client_admin ?
						<Link to={`/projects/${this.props.clientUser.client.id}/client_user/${this.props.clientUser.id}/assign_stores`} className="btn btn-default"><MapMarker/></Link>
						: null
					}
				</td>
			</tr>
...
export default class ClientUserList extends React.Component {
	...
	componentDidMount() {
		fetchClientUsers(this.props.params.clientId).then((clientUsers)=>this.setState({clientUsers}));
	}
	...
	render() {
		...
		let addClientUserLink = `/projects/${this.props.params.clientId}/client_user/add`;
		return (
			<div>
