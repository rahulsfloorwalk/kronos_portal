...
export function fetchStores(clientId){
	return function(dispatch){
		...
		return $.get( url.api_base_path + "client_v1/store", function(stores){
			dispatch({
				type: types.STORE_GET,
				status: "success",
				stores: stores
...
export function fetchStore(storeId){
	return function(dispatch){
		...
		return $.get( url.api_base_path + `client_v1/store/${storeId}`, function(store){
			dispatch({
				type: types.STORE_ID_GET,
				status: "success",
				store: store
...
export function deleteStore(storeId){
	return function(dispatch){
		...
		let req = $.ajax({
			url: url.api_base_path + `client_v1/store/${storeId}`,
			type: "DELETE"
		});
		req.done(function(){
			dispatch({
...
export function saveStoreAddForm(store){
	return function(dispatch){
		...
		var req = $.ajax({
			type: "POST",
			url: url.api_base_path + "client_v1/store",
...
export function saveStoreEditForm(store){
	return function(dispatch){
		...
		var req = $.ajax({
			type: "POST",
			url: url.api_base_path + `client_v1/store/${store.id}`,
			data: JSON.stringify(store),
			contentType: "application/json"
		});
		req.done(function(savedStore){
