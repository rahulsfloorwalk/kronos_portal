...
class SectionCommentViewTestCase(APITestCase):
    fixtures = ['groups']
    ...
    def test_auditor_comment_submit_view_submits_comment(self):
        self.setup_agency()
        self.setup_agency_user()
        self.setup_report_section()
        self.login()
        ...
        self.assertEqual(200, response.status_code)
        self.assertEqual(self.report_section.id, response.data['id'])
        self.assertEqual(self.audit_store.id, response.data['audit_store'])
        self.assertEqual(self.section.id, response.data['section'])
        self.assertEqual(auditor_comment, response.data['auditor_comment'])
