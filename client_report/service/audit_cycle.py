...
def get_audit_cycle_section_averages_for_client(user, questionnaire_type_id, audit_cycle_ids):
    qs = AuditCycle.objects.filter(
        client__id=user.clientuser.client_id,
...
def get_audit_cycle_section_averages(qs, user_id):
    ...
    for audit_cycle in audit_cycles:
        ...
        for section_average in section_averages:
            ...
            cycle_section_data[audit_cycle.id][section.name] = section_average['average']
    sections = []
    for section_name, section in section_data.items():
        cycles = []
        for audit_cycle in audit_cycles:
            cycles.append({
                'audit_cycle_id': audit_cycle.id,
                'audit_cycle_name': audit_cycle.name,
                'value': cycle_section_data[audit_cycle.id].get(section_name)
            })
        sections.append({
