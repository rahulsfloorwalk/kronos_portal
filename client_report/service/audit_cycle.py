...
def get_audit_cycle_section_averages_for_client(client_id):
    audit_cycles = AuditCycle.objects.filter(client__id=client_id).filter(type=AuditCycle.WALKIN).order_by('-end_date')
    section_series = {}
    section_master = []
