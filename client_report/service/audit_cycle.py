...
def get_audit_cycle_section_averages_for_client(user, questionnaire_type_id, audit_cycle_ids):
    qs = AuditCycle.objects.filter(client__id=user.clientuser.client_id, questionnaire_type_id=questionnaire_type_id, status__in=AuditCycle.LIVE_REPORTING_STATUSES)
    if audit_cycle_ids:
        qs = qs.filter(id__in=audit_cycle_ids)
        order_map = {cycle_id:index for index, cycle_id in enumerate(audit_cycle_ids)}
        audit_cycles = sorted(list(qs), key=lambda x: order_map.get(x.id, 9999))
    else:
        audit_cycles = list(qs.order_by('end_date'))
    return get_audit_cycle_section_averages(audit_cycles, user.id)
...
def get_audit_cycle_section_averages(audit_cycles, user_id):
    audit_cycles = list(audit_cycles)
    if not audit_cycles:
        return {'sections':[]}
    cycle_section_data = {}
    section_data = {}
    for audit_cycle in audit_cycles:
        cycle_section_data[audit_cycle.id] = {}
        valid_sections = Section.objects.filter(audit_cycle=audit_cycle.id, questions__visibility=Question.VISIBLE_TO_ALL, questions__hide_question=False, report_sections__not_applicable=False, report_sections__report_section_percentage__isnull=False).distinct()
        section_averages = get_averages_for_sections_for_client_user(valid_sections, user_id)
        for section_average in section_averages:
            section = section_average['section']
            section_key = normalize_section_name(section.name)
            cycle_section_data[audit_cycle.id][section_key] = section_average['average']
            if section_key not in section_data:
                section_data[section_key] = {'section_id':section.id, 'section_name':section.name}
    if len(audit_cycles) > 1:
        common_section_keys = set(cycle_section_data[audit_cycles[0].id].keys())
        for audit_cycle in audit_cycles[1:]:
            common_section_keys.intersection_update(set(cycle_section_data[audit_cycle.id].keys()))
        section_data = {section_key:section for section_key,section in section_data.items() if section_key in common_section_keys}
    sections = []
    for section_key, section in section_data.items():
        cycles = []
        for audit_cycle in audit_cycles:
            cycles.append({'audit_cycle_id':audit_cycle.id, 'audit_cycle_name':audit_cycle.name, 'value':cycle_section_data[audit_cycle.id].get(section_key)})
        sections.append({'section_id':section['section_id'], 'section_name':section['section_name'], 'cycles':cycles})
...
def get_averages_for_sections_for_client_user(sections, user_id):
    ...
    for section in sections:
        if section.max_marks() <= 0:
            continue
        if client_admin:
            filtered_report_sections = ReportSection.objects.filter(audit_store__in=visible_audit_stores, audit_store__status__in=[AuditStore.COMPLETED, AuditStore.ACCEPTED], section=section, not_applicable=False)
        else:
            filtered_report_sections = ReportSection.objects.filter(audit_store__in=visible_audit_stores, audit_store__status__in=[AuditStore.COMPLETED, AuditStore.ACCEPTED], audit_store__audit__store__id__in=non_admin_user_store_list, section=section, not_applicable=False)
        if not filtered_report_sections.exists():
            continue
        average = get_average_for_report_sections(filtered_report_sections)
        if average is not None:
            section_averages.append({'section':section, 'average':average['value']})
...
def get_average_for_report_sections(report_sections):
    ...
    if counter > 0:
        return {'value':int(total / counter)}
    return None
