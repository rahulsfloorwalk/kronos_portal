...
def get_improvable_questions_by_audit_cycle(audit_cycle_ids, questionnaire_type_id, client_user):
    questions = find_by_audit_cycle_ids(audit_cycle_ids).filter(section__audit_cycle__questionnaire_type_id=questionnaire_type_id, visibility=Question.VISIBLE_TO_ALL, hide_question=False).distinct().values('id', 'max_marks', 'section__id', 'section__name', 'question_txt', 'section__audit_cycle_id')
    ...
    for question in questions:
        ...
        if max_marks <= 0:
            continue
        answer_obj = find_answers_by_question_id_for_client(question_id).filter(audit_store__status__in=[AuditStore.COMPLETED, AuditStore.ACCEPTED], not_applicable=False, audit_store__report_sections__section_id=section_id, audit_store__report_sections__not_applicable=False, question__visibility=Question.VISIBLE_TO_ALL, question__hide_question=False, audit_store__audit__audit_cycle_id=cycle_id)
        if not client_admin:
            answer_obj = answer_obj.filter(audit_store__audit__store__id__in=non_admin_user_store_list)
        answer_count = answer_obj.count()
        ...
        if answer_count:
            summary['total_questions'] += answer_count
            summary['total_max_marks'] += total_max_marks
            summary['total_obtained_marks'] += obtained_marks
            if cycle_id not in cycle_summary:
                cycle_summary[cycle_id] = {'audit_cycle_id': cycle_id, 'total_questions': 0, 'total_max_marks': 0, 'total_obtained_marks': 0}
            cycle_summary[cycle_id]['total_questions'] += answer_count
            cycle_summary[cycle_id]['total_max_marks'] += total_max_marks
            cycle_summary[cycle_id]['total_obtained_marks'] += obtained_marks
        key = (section_name, question_txt)
        if key not in question_comparison:
            question_comparison[key] = {'section_id': section_id, 'section_name': section_name, 'question_txt': question_txt, 'cycles': []}
        question_comparison[key]['cycles'].append({'audit_cycle_id': cycle_id, 'question_id': question_id, 'max_marks': total_max_marks, 'obtained_marks': obtained_marks, 'lost_marks': total_max_marks - obtained_marks, 'percentage': percentage})
    summary['percentage'] = round((float(summary['total_obtained_marks']) / summary['total_max_marks']) * 100, 2) if summary['total_max_marks'] else 0
    return {'summary': summary, 'cycle_summary': list(cycle_summary.values()), 'question_comparison': sorted(question_comparison.values(), key=lambda x: sum(c['lost_marks'] for c in x['cycles']), reverse=True)}
