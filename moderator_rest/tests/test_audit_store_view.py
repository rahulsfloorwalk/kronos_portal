...
class AuditStoreIdQARatingTestCase(APITestCase):
    ...
    def test_post_sets_rating(self):
        audit_store = mommy.make(AuditStore, status=AuditStore.SUBMITTED, audit__audit_cycle__status=AuditCycle.ACTIVE,user=self.auditor_user)
        assign_perm('moderator_manage', self.moderator_user, audit_store)
        rating = AuditStore.GOOD

        self.login()

        response = self.client.post(reverse('moderator:audit_store_id_qa_rating_view', kwargs = {
