...
then
	...
	gunicorn --error-logfile ./logs/gunicorn.err.log --access-logfile ./logs/gunicorn.access.log --log-level debug --workers 3 --timeout 180 --bind 0.0.0.0:8000 -p ./logs/gunicorn.pid -D kronos.wsgi
	#GUNICORN_PID=`cat ./logs/gunicorn.pid`;
