...
def sign_up_auditor(data):
    ...
    if mobile_number_service.mobile_number_exists(data.get("phone")):
        response = {'detail': 'a user with this phone number already exists'}
        status = 400
        return response, status

    if data.get("referred_by"):
        if not AdditionalInfo.objects.filter(referral_code=data.get("referred_by").lower()).exists():
    ...
    additional_info.referred_by = data.get("referred_by")
