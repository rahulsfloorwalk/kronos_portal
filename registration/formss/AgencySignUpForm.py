...
from registration.service import agency
...
class AgencySignUpForm(UserCreationForm):
    ...
    agency_name = forms.CharField(max_length=50, required = True)
    ...
    def save(self, commit=True):
        return agency.agency_signup(
            self.cleaned_data["username"],
            self.cleaned_data["password1"],
            self.cleaned_data["agency_name"],
            self.cleaned_data["contact_name"],
            self.cleaned_data["mobile_number"],
            self.cleaned_data["tos_accept"],
        )
