...
class AuditStore(Model):
    ...
    # sentiment_main_keywords = CharField(db_column="main_keywords",max_length= 50000,blank=True, null= True)
    # sentiment_bullet_points = CharField(db_column="bullet_points",max_length= 50000,blank=True, null= True)
    # sentiment_emotions = CharField(db_column="emotions",max_length= 10000,blank=True, null= True)
    # sentiment_positive_words = CharField(db_column="positive_words",max_length= 30000,blank=True, null= True)
    # sentiment_negative_words = CharField(db_column="negative_words",max_length= 30000,blank=True, null= True)
            ...
            if not report_section.not_applicable:
                ...
                for answer in answers:
                   ...
                   # # if not answer.question.question_type == Question.MULTISELECT:
                    ...
                    #         _logger.debug("Report not submittable, some answer is incomplete")
                    #         return False
                    # if answer.question.optional_comment_required is True and answer.question.max_marks >= 1 and answer.marks_obtained==0 and answer.question.question_type == Question.MUTEX and (answer.answer_comment is None or answer.answer_comment.strip() == ''):
                    #     raise AppLogicError("Report not submittable, some required answer comment is incomplete")
                    if answer.question.optional_comment_required is True and (answer.answer_comment is None or answer.answer_comment.strip() == ''):
                        raise AppLogicError("Report not submittable, some required answer comment is incomplete")
                    if not answer.question.question_type == Question.MULTISELECT:
