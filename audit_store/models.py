...
class AuditStore(Model):
    ...
    bullet_points = JSONField(db_column="bullet_points", blank=True, null=True)
    sentiment_emotions = JSONField(db_column="sentiment_emotions", blank=True, null=True)
    sentiment_positive_words = JSONField(db_column="sentiment_positive_words", blank=True, null=True)
            ...
            if not report_section.not_applicable:
                ...
                for answer in answers:
                   ...
                   # # if not answer.question.question_type == Question.MULTISELECT:
                    ...
                    #     raise AppLogicError("Report not submittable, some required answer comment is incomplete")
                    if answer.question.optional_comment_required is True and not answer.not_applicable:
                        if answer.answer_comment is None or answer.answer_comment.strip() == '':
                            raise AppLogicError("Report not submittable, some required answer comment is incomplete")
                    if not answer.question.question_type == Question.MULTISELECT and not answer.not_applicable:
