...
class AuditStoreClientServiceTestCase(TestCase):
    ...
    def test_find_impact_factors_by_id_for_clientuser(self):
        ...
        expect(impact_factors[0]).to(have_keys(name='Foo1', marks_obtained=19, total_marks=49, percentage=39, color_code=1))
        expect(impact_factors[1]).to(have_keys(name='Foo2', marks_obtained=11, total_marks=28, percentage=39, color_code=1))
        expect(impact_factors[2]).to(have_keys(name='Foo3', marks_obtained=3, total_marks=14, percentage=21, color_code=1))
