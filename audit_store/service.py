...
def find_audit_store_by_audit_cycle_id(audit_cycle_id, last_audit_id, status, user_id,qa_user_id,city, start_date, end_date,is_load_more,last_total_count):
    ...
    if status != "" and last_audit_id != "" and start_date and end_date and city:
        ...
        audit_list_obj = Audit.objects.filter( id__in=audit_ids ).order_by( 'id', 'store__city__name', 'store__name' ).select_related( 'store__name', 'store__address', 'store__city__name' ).values( 'id', 'store__name', 'store__address', 'store__city__name', 'count','audit_cycle__client__id' )
    ...
    for audit_report in audit_store_objs:
        ...
        # if audit_report['user__groups__name'] == GROUP_NAME_AUDITOR:
            audit_report_dict['user'] = {
                ...
                'client_id': audit_report.audit.audit_cycle.client.id,
                'agencyuser': None,
                'mobile_numbers': []
            }
        else:
